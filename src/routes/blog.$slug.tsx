      ...
      <article>
        <div className="relative aspect-[21/9] w-full overflow-hidden bg-muted">
          ...
          <div className="container-page relative flex h-full max-w-4xl flex-col justify-end pb-10">
            <Link to="/blog" className="inline-flex w-fit items-center gap-1.5 rounded-full bg-background/85 px-3 py-1.5 text-xs font-semibold text-foreground backdrop-blur hover:text-accent">
        ...
        <div className="container-page max-w-3xl py-10 md:py-14">
          <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-wider text-muted-foreground">
            ...
            <span className="inline-flex items-center gap-1.5">
              ...
              {post.published_at && new Date(post.published_at).toLocaleDateString(ne ? "ne-NP" : "en-US", { month: "long", day: "numeric", year: "numeric" })}
            </span>
            <span className="inline-flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" /> {post.reading_minutes} {t("common.minRead")}</span>
          </div>

          <h1 className={cn("mt-4 text-3xl font-black tracking-tight sm:text-5xl text-balance", showNe && "font-nepali")}>
          ...
          <div className="mt-6 flex items-center gap-3 border-y border-border py-4">
            <div className="grid h-10 w-10 place-items-center rounded-full bg-[image:var(--gradient-primary)] text-xs font-bold text-primary-foreground">HL</div>
            ...
            <div className="flex gap-1">
              {[Twitter, Facebook, Linkedin, Share2].map((Icon, i) => (
                <button key={i} className="grid h-8 w-8 place-items-center rounded-full border border-border text-muted-foreground hover:text-accent">
          ...
          {(showNe ? post.excerpt_ne : post.excerpt_en) && (
            <p className={cn("mt-8 text-xl leading-relaxed text-foreground/80", showNe && "font-nepali")}>
              {showNe ? post.excerpt_ne : post.excerpt_en}
            </p>
          )}

          <div className={cn("prose prose-lg mt-6 max-w-none whitespace-pre-wrap text-base leading-relaxed text-foreground", showNe && "font-nepali")}>
          ...
          {post.tags.length > 0 && (
            <div className="mt-10 flex flex-wrap items-center gap-2 border-t border-border pt-6">
              ...
              {post.tags.map((tg) => (
                <span key={tg} className="rounded-full border border-border bg-card px-3 py-1 text-xs font-semibold text-muted-foreground">
                  #{tg}
                </span>
              ))}
            </div>
        ...
        <section className="border-t border-border bg-surface/40">
          <div className="container-page py-16">
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-3">
              {related.map((r) => (
                <Link key={r.id} to="/blog/$slug" params={{ slug: r.slug }} className="surface-card overflow-hidden">
                  ...
                  <div className="p-5">
                    {r.category && <p className="text-[11px] uppercase tracking-wider text-accent font-bold">{ne ? r.category.name_ne : r.category.name_en}</p>}
                    <h3 className={cn("mt-1.5 text-base font-bold leading-snug", ne && r.lang !== "en" && "font-nepali")}>{ne && r.lang !== "en" ? r.title_ne || r.title_en : r.title_en || r.title_ne}</h3>
                  </div>
                </Link>
              ))}
            </div>
