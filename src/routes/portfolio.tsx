  ...
  head: () => ({
    meta: [
      { title: "Portfolio — Selected Web Projects | Harendra Lamsal" },
      {
        name: "description",
        content:
          "A selection of websites, e-commerce stores, landing pages, and WordPress projects built for clients in Nepal and around the world.",
      },
      { property: "og:title", content: "Portfolio — Harendra Lamsal" },
      {
        property: "og:description",
        content: "Selected web projects: business sites, e-commerce, landing pages, WordPress.",
      },
    ...
    <>
      <section className="hero-bg">
        <div className="container-page py-16 md:py-24">
          ...
          >
            {ne
              ? "साना व्यवसायदेखि स्कूलहरू सम्म — हरेक परियोजनाले फरक समस्या समाधान गर्छ।"
              : "From small businesses to schools — each project solving a different problem."}
          </p>
        </div>
      </section>
          ...
          {visible.map((p) => (
            <article key={p.slug} className="surface-card group overflow-hidden">
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-background/55 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                <div className="absolute right-3 top-3 rounded-full border border-border bg-background/70 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-foreground backdrop-blur">
                  {p.category}
                </div>
                <div className="absolute left-3 top-3 rounded-full border border-border bg-background/70 px-2.5 py-1 text-[10px] font-semibold text-foreground backdrop-blur">
                  {p.year}
                </div>
              </div>
              <div className="p-5">
                ...
                <p
                  className={cn(
                    "mt-1.5 text-sm leading-relaxed text-muted-foreground line-clamp-3",
                    ne && "font-nepali",
                  )}
                >
